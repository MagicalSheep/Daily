...
private:
    ...
    struct node
    {
        ...
        /** Example Begin **/
        T sum1, sum2, sum3, add1, add2, add3;
        /** Example End **/
    } * tree;
...
public:
    ...
    void build(int p, int s, int t)
    {
        ...
        /** Example Begin **/
        tree[p].add1 = tree[p].add2 = tree[p].add3 = 0;
    ...
    // Section update
    void update(int p, int s, int t, T add1, T add2, T add3)
    {
        ...
        if (s <= mid)
            update(p << 1, s, t, add1, add2, add3);
        if (t > mid)
            update(p << 1 | 1, s, t, add1, add2, add3);
        pushup(p);
    }
    ...
    T query(int p, int x)
    {
        ...
        {
            T sum1 = tree[p].sum1 * x * x;
            T sum2 = -tree[p].sum2 * x;
            T sum3 = tree[p].sum3;
...
void modify(int u, int v)
{
    ...
    while (fu != fv)
    {
        ...
        {
            x = l + dfn[u];
            tr.update(1, dfn[fu], dfn[u], 1, 2 * x, x * x);
            l += dep[u] - dep[fu] + 1;
            u = fa[fu], fu = top[u];
        }
        else
        {
            ...
            tr.update(1, dfn[fv], dfn[v], 1, 2 * x, x * x);
            r -= (dep[v] - dep[fv] + 1);
            v = fa[fv], fv = top[v];
        }
    }
    ...
    {
        x = l + dfn[u];
        tr.update(1, dfn[v], dfn[u], 1, 2 * x, x * x);
    }
    else
    {
        x = dfn[fv] - r + (dep[v] - dep[fv] + 1) - 1;
        tr.update(1, dfn[u], dfn[v], 1, 2 * x, x * x);
    }
}
/** Tree Link End **/

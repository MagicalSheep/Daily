...
{
    int n = 5;
    PSegTree::root[0] = PSegTree::build(1, n);

    int a[6] = {0, 6, 2, 5, 3, 4};

    for (int i = 1; i <= n; i++)
        PSegTree::root[i] = PSegTree::update(PSegTree::root[i - 1], 1, n, a[i], 1);
    int ans = PSegTree::query(PSegTree::root[1], PSegTree::root[4], 1, n, 2);
}